...
interface Map<K, V> {
  ...
  $$empty: boolean; // 檢查 Map 物件是否為空，返回布林值
  length: number; // 返回 Map 物件的數量
...
interface Image {
  $base64: (mime: string, size?: number) => string; // 產生圖片的 base64 編碼（默認為 image/jpeg）
  $blob: (mime: string, size?: number) => Blob; // 產生圖片的 Blob 物件（默認為 image/jpeg）
  _download: (mime: string, filename?: string) => void; // 下載圖片
...
interface Element {
  ...
  $i: number; // 返回在父元素中的索引
  $attributes: { [key: string]: string }; // 返回元素的 attributes 的陣列
  $classList: string[]; // 返回元素的 classList 的陣列
  $children: Element[]; // 返回子元素的陣列
  $childNodes: ChildNode[]; // 返回子節點的陣列
  $text: string; // 返回純文字的內容
  $html: string; // 返回元素的 innerHTML
  length: number; // 返回子元素的數量
...
interface DocumentFragment {
  ...
  $children: Element[]; // 返回子元素的陣列
  $childNodes: ChildNode[]; // 返回子節點的陣列
  $html: string; // 返回 DocumentFragment 的字串表示
